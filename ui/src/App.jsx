...
function Viewer({ data, t }) {
  ...
  return <div className="page-stack noc-page"><div className="page-heading"><div><span className="eyebrow">LIVE OPERATIONS</span><h1>{t('viewer')}</h1><p>{t('selectCategory')}</p></div><div className="live-badge"><span className="live-dot"/>{t('monitorLive')}</div></div>
...
function Devices({ data, t, language, refresh }) {
  ...
  const saveDevice = async () => { await query('/update',{ip:editing.ip,name:editing.display_name||editing.name||'',category:editing.category||'',assigned_network:editing.assigned_network||'',scan_profile:editing.scan_profile||'normal',maintenance:editing.maintenance?1:0,mute_alerts:editing.mute_alerts?1:0,pinned:editing.pinned?1:0,critical:editing.critical?1:0,notes:editing.notes||''});await refresh();setEditing(null) }
  ...
  return <div className="page-stack"><div className="page-heading"><div><span className="eyebrow">ASSET INVENTORY</span><h1>{t('devices')}</h1><p>{devices.length} / {data.devices?.length || 0}</p></div></div>
    ...
    {editing && <div className="modal-backdrop" onMouseDown={e=>e.target===e.currentTarget&&setEditing(null)}><div className="modal-card device-editor"><button className="modal-close" onClick={()=>setEditing(null)}><X/></button><span className="eyebrow">DEVICE CONTROL · {editing.ip}</span><h1>{editing.display_name || editing.ip}</h1><div className="detail-grid"><label>{t('name')}<input value={editing.display_name||editing.name||''} onChange={e=>setEditing({...editing,display_name:e.target.value,name:e.target.value})}/></label><label>{t('category')}<input value={editing.category||''} onChange={e=>setEditing({...editing,category:e.target.value})}/></label><label>{t('network')}<select value={editing.assigned_network||''} onChange={e=>setEditing({...editing,assigned_network:e.target.value})}><option value="">—</option>{(data.settingsPayload?.networks||data.status?.networks||[]).map(network=><option value={network} key={network}>{network}</option>)}</select></label><label>{t('scanProfile')}<select value={editing.scan_profile||'normal'} onChange={e=>setEditing({...editing,scan_profile:e.target.value})}><option value="slow">{t('slow')}</option><option value="normal">{t('normal')}</option><option value="fast">{t('fast')}</option></select></label></div><div className="editor-toggles">{[['critical','critical'],['pinned','pinned'],['maintenance','maintenance'],['mute_alerts','muteAlerts']].map(([field,label])=><button className={editing[field]?'active':''} onClick={()=>setEditing({...editing,[field]:!editing[field]})} key={field}><span/><strong>{t(label)}</strong></button>)}</div><label className="notes-field">Notes<textarea value={editing.notes||''} onChange={e=>setEditing({...editing,notes:e.target.value})}/></label>{pingState!=='idle'&&<div className={`ping-feedback ${pingState}`}>{pingState==='running'?<RefreshCw className="spin"/>:pingState==='success'?<CheckCircle2/>:<AlertTriangle/>}<strong>{t(pingState==='running'?'pingRunning':pingState==='success'?'pingSuccess':'pingFailed')}</strong></div>}<div className="modal-actions"><button className="button danger" onClick={()=>action(`/${editing.quarantined?'restore':'remove'}/${encodeURIComponent(editing.ip)}`).then(()=>setEditing(null))}>{t(editing.quarantined?'restore':'quarantine')}</button><button className={`button ping-button ${pingState}`} onClick={pingDevice}>{t('ping')}</button><button className="button" onClick={()=>setEditing(null)}>{t('close')}</button><button className="button primary" onClick={saveDevice}>{t('saveChanges')}</button></div></div></div>}
  </div>
}
...
function SettingsPage({ data, t, language, refresh, enableNotifications }) {
  ...
  const sections=[['general',SlidersHorizontal],['appearance',Eye],['networks',Network],['notification',Bell],['system',Database]]
  return <div className="page-stack settings-page"><div className="page-heading"><div><span className="eyebrow">SYSTEM CONFIGURATION</span><h1>{t('settings')}</h1><p>HOMEii control plane · v{data.status?.version}</p></div></div><div className="settings-layout"><nav className="settings-nav">{sections.map(([key,Icon])=><button className={section===key?'active':''} onClick={()=>setSection(key)} key={key}><Icon/><span>{t(key)}</span><ChevronLeft/></button>)}</nav><section className="panel settings-content">
    {section==='general'&&<><div className="settings-title"><SlidersHorizontal/><div><h2>{t('general')}</h2><p>{t('autoRefresh')} · {t('historyDays')}</p></div></div><div className="form-grid"><label>{t('autoRefresh')}<input type="number" min="10" value={form.auto_refresh} onChange={e=>setForm({...form,auto_refresh:e.target.value})}/><small>10–3600 {t('seconds')}</small></label><label>{t('historyDays')}<input type="number" min="1" max="365" value={form.history_retention_days} onChange={e=>setForm({...form,history_retention_days:e.target.value})}/><small>1–365 {t('days')}</small></label><label>{t('alertProfile')}<select value={form.alert_profile} onChange={e=>setForm({...form,alert_profile:e.target.value})}><option value="quiet">{t('quiet')}</option><option value="normal">{t('normal')}</option><option value="strict">{t('strict')}</option></select></label></div></>}
    {section==='appearance'&&<><div className="settings-title"><Eye/><div><h2>{t('appearance')}</h2><p>{t('language')} · {t('theme')}</p></div></div><div className="choice-grid"><button className={form.language==='he'?'selected':''} onClick={()=>{const next={...form,language:'he'};setForm(next);saveForm(next)}}><Languages/><strong>עברית</strong><span>RTL</span></button><button className={form.language==='en'?'selected':''} onClick={()=>{const next={...form,language:'en'};setForm(next);saveForm(next)}}><Languages/><strong>English</strong><span>LTR</span></button><button className={form.theme==='dark'?'selected':''} onClick={()=>{const next={...form,theme:'dark'};setForm(next);document.documentElement.dataset.theme='dark';saveForm(next)}}><MonitorUp/><strong>{t('dark')}</strong><span>Granite</span></button><button className={form.theme==='light'?'selected':''} onClick={()=>{const next={...form,theme:'light'};setForm(next);document.documentElement.dataset.theme='light';saveForm(next)}}><MonitorUp/><strong>{t('light')}</strong><span>Porcelain</span></button></div></>}
    {section==='networks'&&<><div className="settings-title"><Network/><div><h2>{t('networks')}</h2><p>{data.settingsPayload?.networks?.length||0} {t('networks')}</p></div></div><div className="network-list">{(data.settingsPayload?.networks||data.status?.networks||[]).map((network,index)=><div key={network}><span className="icon-box"><Network/></span><div><strong>{data.settingsPayload?.network_names?.[network]||`Network ${index+1}`}</strong><code>{network}</code></div><span className="healthy-label">{t('monitorLive')}</span></div>)}</div></>}
    ...
    {section==='system'&&<><div className="settings-title"><DatabaseIcon/><div><h2>{t('system')}</h2><p>{t('database')} & workers</p></div></div><dl className="system-dl"><div><dt>Version</dt><dd>{data.status?.version}</dd></div><div><dt>{t('database')}</dt><dd className={data.status?.db_ok?'healthy-label':'danger-text'}>{data.status?.db_ok?t('ready'):t('unavailable')}</dd></div><div><dt>{t('networks')}</dt><dd>{data.status?.networks?.length||0}</dd></div><div><dt>PWA</dt><dd>{'serviceWorker' in navigator?t('ready'):t('unavailable')}</dd></div></dl></>}
...
export default function App() {
  ...
  const pages={dashboard:<Dashboard data={data} t={t} setRoute={setRoute} language={language}/>,viewer:<Viewer data={data} t={t}/>,devices:<Devices data={data} t={t} language={language} refresh={refresh}/>,alerts:<Alerts data={data} t={t} language={language} refresh={refresh}/>,history:<HistoryPage history={data.history} t={t} language={language}/>,tools:<Tools data={data} t={t}/>,settings:<SettingsPage data={data} t={t} language={language} refresh={refresh} enableNotifications={enableNotifications}/>}
