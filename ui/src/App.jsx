...
import {
  ...
  LogOut, LockKeyhole, UserPlus, Users,
} from 'lucide-react'
import {
...
function Devices({ data, t, language, refresh, initialFilter = '' }) {
  ...
  return <div className="page-stack"><div className="page-heading"><div><span className="eyebrow">{t('assetInventory')}</span><h1>{t('devices')}</h1><p>{devices.length} {t('of')} {data.devices?.length || 0}</p></div></div>{notice&&<div className="success-banner"><CheckCircle2/>{notice}</div>}
    ...
    <section className="device-grid">{devices.slice(0,visibleCount).map(device => <article className={`device-card state-${device.status}`} key={device.ip}>
    ...
    </article>)}{!devices.length&&<Empty t={t}/>}</section>{devices.length>visibleCount&&<button className="button load-more" onClick={()=>setVisibleCount(count=>count+48)}>{t('showMore')} · {devices.length-visibleCount}</button>}
...
function AuthScreen({ setupRequired, onAuthenticated }) {
  const [form,setForm]=useState({username:'',password:'',display_name:''}); const [error,setError]=useState(''); const [busy,setBusy]=useState(false)
  const submit=async(event)=>{event.preventDefault();setBusy(true);setError('');try{await api(setupRequired?'/auth/setup':'/auth/login',{method:'POST',body:JSON.stringify(form)});await onAuthenticated()}catch(e){setError(e.message)}finally{setBusy(false)}}
  return <main className="auth-screen"><section className="auth-card"><Logo/><div className="auth-intro"><span className="eyebrow">SECURE NETWORK INTELLIGENCE</span><h1>{setupRequired?'יצירת מנהל ראשי':'כניסה מאובטחת'}</h1><p>{setupRequired?'הגדר את חשבון המנהל הראשון. הסיסמה חייבת להכיל לפחות 8 תווים.':'הזן שם משתמש וסיסמה כדי להיכנס למערכת.'}</p></div><form onSubmit={submit}>{setupRequired&&<label>שם תצוגה<input autoComplete="name" value={form.display_name} onChange={e=>setForm({...form,display_name:e.target.value})}/></label>}<label>שם משתמש<input autoFocus autoComplete="username" required minLength="3" value={form.username} onChange={e=>setForm({...form,username:e.target.value})}/></label><label>סיסמה<input type="password" autoComplete={setupRequired?'new-password':'current-password'} required minLength="8" value={form.password} onChange={e=>setForm({...form,password:e.target.value})}/></label>{error&&<div className="auth-error"><AlertTriangle/>{error}</div>}<button className="button primary auth-submit" disabled={busy}>{busy?<RefreshCw className="spin"/>:<LockKeyhole/>}{setupRequired?'צור חשבון מנהל':'כניסה'}</button></form><small className="auth-note">HOMEii protects the web console with an encrypted session.</small></section></main>
}

function UserManagement({ t, currentUser }) {
  const [users,setUsers]=useState([]); const [draft,setDraft]=useState({username:'',display_name:'',password:'',role:'user',viewer_edge_to_edge:false}); const [message,setMessage]=useState('')
  const load=()=>api('/admin/users').then(result=>setUsers(result.users||[])).catch(error=>setMessage(error.message))
  useEffect(()=>{load()},[])
  const create=async()=>{await api('/admin/users',{method:'POST',body:JSON.stringify(draft)});setDraft({username:'',display_name:'',password:'',role:'user',viewer_edge_to_edge:false});setMessage(t('userCreated'));load()}
  const update=async(user,next)=>{await api(`/admin/users/${user.id}`,{method:'PATCH',body:JSON.stringify({...user,...next})});load()}
  return <div className="users-panel"><div className="settings-title"><Users/><div><h2>{t('userManagement')}</h2><p>{t('userManagementHelp')}</p></div></div><div className="user-create-card"><div className="form-grid"><label>{t('username')}<input value={draft.username} onChange={e=>setDraft({...draft,username:e.target.value})}/></label><label>{t('displayName')}<input value={draft.display_name} onChange={e=>setDraft({...draft,display_name:e.target.value})}/></label><label>{t('password')}<input type="password" value={draft.password} onChange={e=>setDraft({...draft,password:e.target.value})}/></label><label>{t('role')}<select value={draft.role} onChange={e=>setDraft({...draft,role:e.target.value})}><option value="admin">Admin</option><option value="user">User</option><option value="viewer">Viewer</option></select></label></div><button type="button" className={`setting-toggle ${draft.viewer_edge_to_edge?'active':''}`} onClick={()=>setDraft({...draft,viewer_edge_to_edge:!draft.viewer_edge_to_edge})}><span/><div><strong>{t('edgeToEdge')}</strong><small>{t('edgeToEdgeHelp')}</small></div></button><button className="button primary" onClick={create} disabled={draft.username.length<3||draft.password.length<8}><UserPlus/>{t('addUser')}</button></div>{message&&<div className="success-banner">{message}</div>}<div className="user-list">{users.map(user=><article key={user.id}><div className="user-avatar">{(user.display_name||user.username).slice(0,1).toUpperCase()}</div><div><strong>{user.display_name||user.username}</strong><small>@{user.username} · {user.role}</small></div><select value={user.role} disabled={user.id===currentUser?.id} onChange={e=>update(user,{role:e.target.value})}><option value="admin">Admin</option><option value="user">User</option><option value="viewer">Viewer</option></select><button className={`mini-toggle ${user.viewer_edge_to_edge?'active':''}`} onClick={()=>update(user,{viewer_edge_to_edge:!user.viewer_edge_to_edge})}>{t('edgeToEdge')}</button><button className={`mini-toggle ${user.active?'active':''}`} disabled={user.id===currentUser?.id} onClick={()=>update(user,{active:!user.active})}>{user.active?t('active'):t('disabled')}</button></article>)}</div></div>
}

function SettingsPage({ data, t, language, refresh, enableNotifications, currentUser }) {
  const settings=data.settings||{}; const [form,setForm]=useState({language:settings.language||language,theme:settings.theme||'granite',auto_refresh:settings.auto_refresh||'30',history_retention_days:settings.history_retention_days||'30',alert_profile:settings.alert_profile||'normal',auto_restore_quarantined:settings.auto_restore_quarantined||'1',default_view:settings.default_view||'table',dashboard_style:settings.dashboard_style||'advanced',status_animation:settings.status_animation||'blink',discovery_mode:data.settingsPayload?.discovery_mode||'auto_manual',discovery_protocols:data.settingsPayload?.discovery_protocols||['ping','arp','dns','special','vendor']})
  const [section,setSection]=useState('general')
  const [transfer,setTransfer]=useState({type:'',message:''})
  const [reconcile,setReconcile]=useState(null)
  const [networksText,setNetworksText]=useState('')
  const importRef=useRef(null)
  useEffect(()=>setForm(f=>({...f,language:settings.language||f.language,theme:settings.theme||f.theme,auto_refresh:settings.auto_refresh||f.auto_refresh,history_retention_days:settings.history_retention_days||f.history_retention_days,alert_profile:settings.alert_profile||f.alert_profile,auto_restore_quarantined:settings.auto_restore_quarantined||f.auto_restore_quarantined,default_view:settings.default_view||f.default_view,dashboard_style:settings.dashboard_style||f.dashboard_style,status_animation:settings.status_animation||f.status_animation,discovery_mode:data.settingsPayload?.discovery_mode||f.discovery_mode,discovery_protocols:data.settingsPayload?.discovery_protocols||f.discovery_protocols})),[settings,data.settingsPayload?.discovery_mode,data.settingsPayload?.discovery_protocols])
  ...
  const runReconciliation=async()=>{if(!window.confirm(t('reconcileConfirm')))return;setReconcile({running:true});try{await api('/admin/reconcile-inventory',{method:'POST'});const poll=async()=>{const result=await api('/admin/reconcile-inventory');const state=result.reconciliation||{};setReconcile(state);if(state.running)setTimeout(poll,1500);else await refresh()};setTimeout(poll,1000)}catch(error){setReconcile({running:false,error:error.message})}}
  ...
  const sections=[['general',SlidersHorizontal],['appearance',Eye],['discovery',Radar],['networks',Network],['notification',Bell],['users',Users],['system',Database],['dataManagement',FileJson]]
  return <div className="page-stack settings-page"><div className="page-heading"><div><span className="eyebrow">{t('systemConfiguration')}</span><h1>{t('settings')}</h1><p>{t('controlPlane')} · v{data.status?.version}</p></div></div>{transfer.message&&<div className={transfer.type==='error'?'error-banner':'success-banner'}>{transfer.type==='error'?<AlertTriangle/>:<CheckCircle2/>}{transfer.message}<button onClick={()=>setTransfer({type:'',message:''})}><X/></button></div>}<div className="settings-layout"><nav className="settings-nav">{sections.map(([key,Icon])=><button className={section===key?'active':''} onClick={()=>setSection(key)} key={key}><Icon/><span>{t(key)}</span><ChevronLeft/></button>)}</nav><section className="panel settings-content">
    ...
    {section==='discovery'&&<><div className="settings-title"><Radar/><div><h2>{t('discovery')}</h2><p>{t('discoveryHelp')}</p></div></div><div className="form-grid"><label>{t('discoveryMode')}<select value={form.discovery_mode} onChange={e=>setForm({...form,discovery_mode:e.target.value})}><option value="auto_manual">{t('autoManual')}</option><option value="auto_only">{t('autoOnly')}</option><option value="manual_only">{t('manualOnly')}</option></select><small>{t('discoveryModeHelp')}</small></label></div><div className="protocol-grid">{['ping','arp','dns','special','vendor'].map(protocol=><button type="button" className={(form.discovery_protocols||[]).includes(protocol)?'active':''} onClick={()=>setForm({...form,discovery_protocols:(form.discovery_protocols||[]).includes(protocol)?form.discovery_protocols.filter(item=>item!==protocol):[...(form.discovery_protocols||[]),protocol]})} key={protocol}><span/><div><strong>{t(`protocol_${protocol}`)}</strong><small>{t(`protocol_${protocol}_help`)}</small></div></button>)}</div><div className="admin-reconcile-card"><div className="admin-reconcile-head"><ShieldAlert/><div><span className="eyebrow">{t('adminOnly')}</span><h3>{t('reconcileTitle')}</h3><p>{t('reconcileHelp')}</p></div></div><button type="button" className={`setting-toggle ${form.auto_restore_quarantined==='1'?'active':''}`} onClick={()=>setForm({...form,auto_restore_quarantined:form.auto_restore_quarantined==='1'?'0':'1'})}><span/><div><strong>{t('autoRestore')}</strong><small>{t('autoRestoreHelp')}</small></div></button>{reconcile&&<div className={`reconcile-status ${reconcile.error?'failed':reconcile.running?'running':'complete'}`}>{reconcile.running?<><RefreshCw className="spin"/><strong>{t('reconcileRunning')}</strong></>:reconcile.error?<><AlertTriangle/><strong>{reconcile.error}</strong></>:<><CheckCircle2/><div><strong>{t('reconcileComplete')}</strong><span>{reconcile.online||0} {t('online')} · {reconcile.quarantined||0} {t('quarantined')} · {reconcile.restored||0} {t('restored')}</span></div></>}</div>}<button type="button" className="button danger" disabled={reconcile?.running} onClick={runReconciliation}><Radar/>{reconcile?.running?t('scanning'):t('runReconcile')}</button></div></>}    {section==='networks'&&<><div className="settings-title"><Network/><div><h2>{t('networks')}</h2><p>{t('networksHelp')}</p></div></div><label className="notes-field">{t('networkRanges')}<textarea value={networksText} onChange={e=>setNetworksText(e.target.value)} placeholder={'192.168.1.0/24\n10.0.0.0/24'}/><small>{t('oneNetworkPerLine')}</small></label><div className="settings-actions inline"><button className="button primary" onClick={saveNetworks}>{t('saveNetworks')}</button></div><div className="network-list">{(data.settingsPayload?.networks||data.status?.networks||[]).map((network,index)=><div key={network}><span className="icon-box"><Network/></span><div><strong>{data.settingsPayload?.network_names?.[network]||`${t('network')} ${index+1}`}</strong><code>{network}</code></div><span className="healthy-label">{t('monitorLive')}</span></div>)}</div></>}
    ...
    {section==='users'&&<UserManagement t={t} currentUser={currentUser}/>}
...
export default function App() {
  ...
  const [auth,setAuth]=useState({loading:true,authenticated:false,setup_required:false,user:null})
  ...
  const loadAuth=async()=>{try{const result=await api('/auth/session');setAuth({...result,loading:false});if(result.authenticated){if(result.user?.role==='viewer')setRoute('viewer');await refresh()}}catch{setAuth({loading:false,authenticated:false,setup_required:false,user:null})}}
  ...
  useEffect(()=>{loadAuth()},[])
  useEffect(()=>{if(!auth.authenticated)return;const interval=Math.max(10,Number(data.settings?.auto_refresh||30))*1000;const id=setInterval(refresh,interval);return()=>clearInterval(id)},[auth.authenticated,data.settings?.auto_refresh])
  ...
  if(auth.loading)return <div className="loading-screen"><Logo/><div className="loader"/></div>
  if(!auth.authenticated)return <AuthScreen setupRequired={auth.setup_required} onAuthenticated={loadAuth}/>
  const role=auth.user?.role||'viewer'; const edgeViewer=role==='viewer'&&auth.user?.viewer_edge_to_edge
  ...
  const allowed={admin:navItems,user:navItems.filter(([key])=>!['tools','settings'].includes(key)),viewer:navItems.filter(([key])=>key==='viewer')}[role]||[]
  const logout=async()=>{await api('/auth/logout',{method:'POST'});setData({});setAuth({loading:false,authenticated:false,setup_required:false,user:null})}
  return <div className={`app-shell role-${role} ${edgeViewer?'edge-viewer':''}`}><aside className={menuOpen?'open':''}><Logo/><details className="account-menu"><summary className="account-chip"><span>{(auth.user.display_name||auth.user.username).slice(0,1).toUpperCase()}</span><div><strong>{auth.user.display_name||auth.user.username}</strong><small>{t(`${role}Mode`)}</small></div><ChevronLeft/></summary><div className="account-popover"><button onClick={logout}><LogOut/><span>{t('logout')}</span></button></div></details><nav>{allowed.map(([key,Icon])=><button key={key} className={route===key?'active':''} onClick={()=>{setRoute(key);setMenuOpen(false)}}><Icon/><span>{t(key)}</span>{route===key&&<i/>}</button>)}</nav><div className="sidebar-health"><span className="live-dot"/><div><strong>{t('monitorLive')}</strong><small>v{data.status?.version||'6.3.1'}</small></div></div></aside><div className="main-shell"><header><button className="mobile-menu icon-button" onClick={()=>setMenuOpen(!menuOpen)}><Menu/></button><div><span className="eyebrow">HOMEii / {t(route)}</span><h2>{new Date().toLocaleDateString(language==='he'?'he-IL':'en-US',{weekday:'long',day:'numeric',month:'long'})}</h2></div><div className="header-actions"><button className="button subtle" onClick={refresh}><RefreshCw className={loading?'spin':''}/><span>{t('refresh')}</span></button>{role==='admin'&&<button className="button primary" onClick={async()=>{await api('/scan?mode=manual');refresh()}}><Radar/><span>{t('scanNow')}</span></button>}</div></header>{error&&<div className="error-banner"><AlertTriangle/>{error}<button onClick={()=>setError('')}><X/></button></div>}<main>{loading&&!data.status?<div className="loading-screen"><Logo/><div className="loader"/></div>:pages[route]||pages[allowed[0]?.[0]]}</main></div>{menuOpen&&<button className="scrim" onClick={()=>setMenuOpen(false)}/>}</div>
}
