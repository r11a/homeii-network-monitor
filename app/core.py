...
from app.state_machine import next_probe_status
...
def live_change_token() -> str:
    """Return a cheap fingerprint for UI-relevant state changes."""
    conn = db()
    try:
        device_state = conn.execute(
            "SELECT COUNT(*), COALESCE(MAX(updated_at), 0), "
            "COALESCE(SUM(CASE status WHEN 'online' THEN 1 WHEN 'offline' THEN 10 "
            "WHEN 'unstable' THEN 100 WHEN 'new' THEN 1000 ELSE 0 END), 0) FROM devices"
        ).fetchone()
        alert_state = conn.execute(
            "SELECT COUNT(*), COALESCE(MAX(updated_at), 0) FROM alerts"
        ).fetchone()
        event_state = conn.execute("SELECT COALESCE(MAX(id), 0) FROM events").fetchone()
        return ":".join(str(value or 0) for value in (*device_state, *alert_state, *event_state))
    finally:
        conn.close()
